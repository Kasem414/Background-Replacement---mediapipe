...
def update_frame():
    ...
    # Convert mask to binary condition (True = person, False = background)
    condition = mask > 0.5
    condition = np.stack((condition,) * 3, axis=-1)  # Match shape with image channels
    # Composite output: Keep person pixels, replace background pixels
